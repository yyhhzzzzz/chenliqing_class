...
- [x] 搭建 Flask 轻量化 Web 应用，支持图片拖拽/文件选择上传
- [x] 实现 YOLOv8 核心推理逻辑与二分类（听讲中/分心中）状态映射标注
- [x] 将细分动作与专注判定状态全面进行中文化显示（前台诊断明细表格）
- [x] 重构项目目录结构，实现模块化（主程序 `main.py` 与检测核心 `src/detector.py`）
- [x] 整理权重文件存放路径，将最佳模型移动至 `weights/best.pt`
- [x] 优化运行时环境，屏蔽 YOLO 默认在根目录下自动创建 empty `runs/` 目录的垃圾文件行为
- [ ] 后续人脸考勤打卡模块与可视化分析看板开发
