...
def train_model():
    ...
    results = model.train(
        ...
        epochs=1,          # 既然有1天时间，直接拉到 300 轮（配合 patience 早停机制，绝不会白白浪费时间）
        patience=50,         # 50 轮内 mAP 精度不再提升，则自动提前终止训练，防止过拟合
    
    # 👁️ 【核心精度调优：分辨率与批次的博弈】
        imgsz=1024,          # 🔥 关键提升！将分辨率从 640 提升到 1024，大幅增强后排小人头/低头动作的识别率
        batch=8,             # ⚠️ 配合 1024 分辨率，8GB 显存极限大概只能开到 batch=8 (如果报错 OOM，请降为 4)
    
    # ⚙️ 【系统性能调优：榨干 IO 速度】
        device=0,            # 使用首张 GPU
        workers=4,           # Windows 下设为 4。如果训练中出现 DataLoader 报错，再改回 0
